...
export const useGameStore = create<GameState>()(
  persist(
    (set, get) => ({
      ...
      addPlay: (slug) =>
        set((s) => ({
          playCounts: { ...s.playCounts, [slug]: (s.playCounts[slug] ?? 0) + 1 },
        })),

      getPlayCount: (slug) => get().playCounts[slug] ?? 0,

      clearGameData: () =>
        set({ currentGameId: null, highScores: {}, gamesPlayed: 0, playCounts: {} }),
    }),
    { name: "nya-hub-games" }
  )
);